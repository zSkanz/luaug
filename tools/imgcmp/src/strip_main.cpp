...
constexpr std::string_view kUsage =
    "usage: imgstrip <out.png> <in0.png> <in1.png> [...]\n"
...
int main(int argc, char** argv)
{
    ...
    for (std::size_t index = 1; index < args.size(); ++index)
    {
        ...
        // frames.
        if (!frames.empty() && (loaded.image.width != frames.front().width
                                   || loaded.image.height != frames.front().height))
        {
    ...
    luaug::imgcmp::Image strip =
        luaug::imgcmp::makeImage(frameWidth * static_cast<int>(frames.size()), height, 0xFF);
