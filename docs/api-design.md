...
| 24 | RemoteEvent/RemoteFunction | v1: none; `@std/net` primitives; replication reserved. **`RemoteEvent` is built (ADR 0077)** over the replication engine, with its events named as past-tense facts: `ServerReceived` and `ClientReceived`. A remote function, which waits for an answer, is not. `@std/net` stays PARALLEL to replication and not underneath: one talks to the author's backend, the other to the players in the match | Honest scope; portable backends (ADR 0012) |
