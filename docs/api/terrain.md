...
| `MinHeight` | `number` | `-256` | read/write | The world's floor, in metres: no voxel below it is ever written.<br><br>Ground laid where there was none -- by `WriteHeights`, Generate Flat Ground or a first raise on empty terrain -- is a slab 32 metres deep under its lowest surface, rounded down to a whole chunk, and never deeper than this. |
