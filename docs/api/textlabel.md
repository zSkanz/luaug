...
| `TextXAlignment` | `Enum.HorizontalAlignment` | `Enum.HorizontalAlignment.Center` | read/write | Where the text sits across the box: `Left`, `Center` or `Right`. Named for the text, because a layout's `HorizontalAlignment` places children and this places words; the values are the same enum a layout uses. |
| `TextYAlignment` | `Enum.VerticalAlignment` | `Enum.VerticalAlignment.Center` | read/write | Where the text sits down the box: `Top`, `Center` or `Bottom`. |
