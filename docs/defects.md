...
| D016 | agent, 2026-08-20 | `BindToClose` has no capped grace period: a close handler that yields is cut off at the next drain rather than waited for. `architecture.md` §app promises the cap | scheduled | M5, with the shutdown ordering physics will need. Found by the §app audit in `docs/briefs/m4.5-kickoff.md` |
