...
| **G2** | The absence paragraphs move **in the same commit that makes them false**, not before — `docs/api-design.md:479-492` says that paragraph "stays until each one ships rather than being edited in advance". `docs/manual/migrating/not-here.md:26` ("Terrain \| **Planned**, next phase. The open question is authoring.") goes. The roadmap gains the F1 detail section. | `docs/api-design.md`, `docs/manual/migrating/not-here.md`, `docs/roadmap.md`, `PROGRESS.md`, `docs/briefs/f1-kickoff.md` | Docs. | doc-lint |
