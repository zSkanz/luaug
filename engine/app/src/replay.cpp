...
std::optional<core::EngineError> runScenario(const ReplayScenario& scenario, ReplayTrace& out)
{
    ...
    u64 errors = 0;
    core::LogSink previous = core::setLogSink(
        [&errors](LogLevel level, std::string_view text)
        {
    ...
    if (errors != 0)
    {
        const std::array<I18nArg, 2> args{
            I18nArg{"name", scenario.name}, I18nArg{"count", static_cast<core::i64>(errors)}};
...
std::optional<core::EngineError> runReplayGate(const std::filesystem::path& root, bool record)
{
    ...
    for (const std::filesystem::path& directory : directories)
    {
        ...
        if (record)
        {
            ...
            const std::array<I18nArg, 2> args{
                I18nArg{"name", scenario.name}, I18nArg{"path", tracePath.string()}};
        ...
        {
            const std::array<I18nArg, 2> args{
                I18nArg{"path", tracePath.string()}, I18nArg{"platform", platformName()}};
