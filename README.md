...
> **STATUS: pre-alpha — docs-first.** The engine is not built yet. This repository currently contains the complete architecture, API design, roadmap, and research package. The engine is being built autonomously, milestone by milestone, by an AI agent (Claude Opus, multi-agent orchestration) following [`MASTER_PROMPT.md`](MASTER_PROMPT.md), with human review at every milestone gate.
